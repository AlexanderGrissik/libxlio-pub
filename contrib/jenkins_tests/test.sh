...
if [ ! -z "${test_remote_ip}" ] ; then
	...
	if [ -z "${skip_remote_prep}" ] ; then
		...
		if [[ "${rmt_os}" =~ .*"${local_os}".* ]] ; then
			...
			eval "pid=$(${sudo_cmd} ssh ${rmt_user}@${test_remote_ip} pidof vmad)"
			if [ ! -z "${pid}" ] ;  then 
				echo "vmad pid=${pid}"
				eval "${sudo_cmd} ssh ${rmt_user}@${test_remote_ip} kill -9 ${pid}"
			fi
			${sudo_cmd} scp -q ${install_dir}/sbin/vmad ${rmt_user}@${test_remote_ip}:${sperf_exec_dir}
			eval "${sudo_cmd} ssh ${rmt_user}@${test_remote_ip} ${sudo_cmd} ${sperf_exec_dir}/vmad &"
		else

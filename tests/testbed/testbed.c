	...
	while (!_done) {
		...
		if (conn) {
			if (conn->vma_buf && (conn->vma_buf_offset < conn->vma_buf->len)) {
				...
				_xlio_api->socketxtreme_free_packets(&conn->vma_packet, 1);
				conn->vma_buf = NULL;
				conn->vma_buf_offset = 0;
				conn = NULL;
				n = 0;
		...
		while (0 == n) {
			...
			n = _xlio_api->socketxtreme_poll(_vma_ring_fd, &vma_comps, 1, 0);
			if (n > 0) {
...
static int _tcp_client_init(struct sockaddr_in *addr)
{
	...
	while (0 == rc) {
		uint32_t event;
		struct xlio_socketxtreme_completion_t vma_comps;
		rc = _xlio_api->socketxtreme_poll(_vma_ring_fd, &vma_comps, 1, 0);
		if (rc > 0) {
			event = (uint32_t)vma_comps.events;
